...
import 'package:pharma/data/model/gst_model.dart';
import 'package:pharma/data/model/pan_model.dart';
import 'package:pharma/query/seller/global.dart';

Future<dynamic> getGstPanMethod(
    BuildContext context, String type, String panOrGstNo) async {
  try {
    var data = await SellerGlobalHandler.requestGet(
        '/verify/pan/gst/$type/$panOrGstNo');
    print(data.body);
    var jsonData = jsonDecode(data.body);
    if (data.statusCode == 200) {
      if (type == 'PAN') {
        var res = PanModel.fromJson(jsonData);
        print(res);
        return res;
      } else if (type == 'GST') {
        var res = GstModel.fromJson(jsonData);
        print(res);
        return res;
      }
    }
...
// Future<PanGstModel?> getGstPanMethod(BuildContext context,
//     {required String userType, required PanGstModel body}) async {
//   try {
...
//     PanGstModel response = PanGstModel.fromJson(jsonData);
